...
class GoldPriceBubble:
    ...
    def _get_gold_price(self):
        try:
            url = "https://ms.jr.jd.com/gw/generic/jimu/h5/m/homeFeedFlow?reqData={%22tagId%22:20289,%22contentId%22:%2213245%22}"
            headers = {
                "User-Agent": "jdapp;iPhone;12.3.4;14.0;869443e5542b8b6f;network/wifi;model/iPhone10,3;appBuild/9999;Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X)"
            }
            response = requests.get(url, headers=headers, timeout=5)
            data = response.json()
            items = data['resultData']['resultList']
            gold_items = next(item for item in items if item.get("stockQuoteVos"))
            quotes = gold_items['stockQuoteVos']

            inner_q = next((q for q in quotes if q['uniqueCode'] == "SGE-Au(T+D)"), None)
            outer_q = next((q for q in quotes if q['uniqueCode'] == "WG-XAUUSD"), None)

            if inner_q and outer_q:
                inner_price = inner_q['lastPrice']
                outer_price = outer_q['lastPrice']
                return f"{inner_price:.2f}", f"{outer_price:.2f}"
        except Exception as e:
            logger.warning(f"金价获取失败：{e}")
    ...
    def _update_loop(self):
        while True:
            ...
            if inner:
                self.canvas.itemconfig(self.inner_text, text=f"上海金:  {inner} ¥")
            if outer:
                self.canvas.itemconfig(self.outer_text, text=f"伦敦金:  {outer} $")
            time.sleep(2)
    ...
    def _start_tray_icon(self):
        ...
        def show_about(icon, item):
            msg.showinfo("关于", "GoldPriceBubble v1.6\n将金价获取频率提高为2秒一次\n作者：Lucas Lee\n数据2秒更新一次")
